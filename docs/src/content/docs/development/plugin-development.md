...
An important one is [core.js](https://github.com/conversejs/.js/blob/master/src/headless/core.js),
which is responsible for bootstrapping the plugin architecture,
setting up and maintaining the connection to the XMPP
server and declaring the public and protected APIs.

The other non-plugin files all contain utility methods in
[src/utils](https://github.com/conversejs/converse.js/blob/master/src/utils) and
[src/headless/utils](https://github.com/conversejs/converse.js/blob/master/src/headless/utils).
...
[pluggable.js](https://github.com/jcbrand/pluggable.js/), provides you
...
There is a [Yeoman](http://yeoman.io/) code generator, called
[generator-conversejs](https://github.com/jcbrand/generator-conversejs), which
you can use to generate plugin scaffolding/boilerplate that serves as a
starting point and basis for writing your plugin.

Please refer to the [generator-conversejs](https://github.com/jcbrand/generator-conversejs)
...
To whitelist a plugin simply means to specify [`whitelisted_plugins`](/configuration/#whitelisted_plugins) when
...
[./src/headless/converse-core.js](https://github.com/jcbrand/converse.js/blob/master/src/headless/converse-core.js)
or the `WHITELISTED_PLUGINS` array in [./src/converse.js](https://github.com/jcbrand/converse.js/blob/master/src/converse.js).
...
Converse uses [lit-html](https://lit-html.polymer-project.org/guide)
templates and templates are imported as separate files.

It's possible to configure your module bundler (e.g. Webpack) in such a way that a
...
These elements are derived from the `CustomElement` class, and declared using `api.elements.define`.
...
:::caution
Using the `overrides` feature from pluggable.js is discouraged. It's
much better to use events, promises and [hooks](#hooks) to modify the behaviour of
Converse.
...
newer classes and objects, such as the web components. For these classes,
...
A better approach is to use the events and [hooks](#hooks) emitted by Converse, and to add
...
these settings with the `_converse.api.settings.extend` method.

## Exposing promises

Converse has a `waitUntil` API method
...
`_converse.api.trigger`, which not
...
[src/converse-bookmarks.js](https://github.com/jcbrand/converse.js/blob/6c3aa34c23d97d679823a64376418cd0f40a8b94/src/converse-bookmarks.js#L528)).
...
If you need new events or promises, then
[please open an issue or make a pull request on Github](https://github.com/jcbrand/converse.js).
...
generated by [generator-conversejs](https://github.com/jcbrand/generator-conversejs).
