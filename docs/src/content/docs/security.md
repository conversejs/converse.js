...
```http
Content-Security-Policy: script-src 'self' 'wasm-unsafe-eval' https://cdn.conversejs.org;
```

The `'wasm-unsafe-eval'` source is required for OMEMO end-to-end encryption,
which uses a WebAssembly crypto module (libomemo). It is much narrower than
`'unsafe-eval'`: it permits WebAssembly compilation only, while still forbidding
string evaluation via `eval()` or `new Function()`. It is supported by every
browser that enforces CSP on WebAssembly, so there is no browser gap.
Omit it only if you disable OMEMO entirely.

This directive covers `script-src` only. Converse loads avatars, shared
media and link previews from arbitrary remote origins and uploads to file
services, so locking down `img-src`, `media-src` or `connect-src` will break
those features unless you scope them to the origins you actually use.
