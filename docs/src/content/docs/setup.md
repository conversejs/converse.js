...
description: How to set up and integrate Converse with your XMPP server and website.
---
...
At the very least you'll need Converse and an [XMPP server](#an-xmpp-server) with
[websocket](#websocket) or [BOSH](#bosh-xmpp-over-http) enabled. That's definitely
...
![A diagram of a possible setup, consisting of Converse, a web server, a backend web application, an XMPP server, a user directory such as LDAP and an XMPP server.](../../assets/diagram.png)

*This diagram shows the various services in a fairly common setup (image generated with [draw.io](https://draw.io)).*

## The various components

### An XMPP server

Converse uses [XMPP](https://xmpp.org/about-xmpp/) as its
...
and a list of servers that you can set up yourself on [xmpp.org](https://xmpp.org/xmpp-software/servers/).
...
BOSH can be seen as XMPP-over-HTTP. In other words, it allows for XMPP
stanzas to be sent over an HTTP connection.

HTTP connections are stateless and usually short-lived.
...
protocol is defined in [XEP-0206: XMPP Over BOSH](https://xmpp.org/extensions/xep-0206.html).

Popular XMPP servers such as
ejabberd [(mod_bosh)](https://docs.ejabberd.im/admin/configuration/modules/#mod_bosh),
Prosody [(mod_bosh)](http://prosody.im/doc/setting_up_bosh) and
[OpenFire](http://www.igniterealtime.org/projects/openfire/) all include
...
For a standalone manager, see for example [Punjab](https://github.com/twonds/punjab)
and [node-xmpp-bosh](https://github.com/dhruvbird/node-xmpp-bosh).

The demo on the [Converse homepage](https://conversejs.org) uses a connection
...
Let's say the domain under which you host Converse is *example.org:80*,
but the domain of your connection manager or the domain of
your HTTP file server (for [XEP-0363 HTTP File Upload](https://xmpp.org/extensions/xep-0363.html))
...
[same-origin security policy](https://developer.mozilla.org/en-US/docs/Web/Security/Same-origin_policy).
...
and your connection manager is running at `someothersite.com/http-bind`.
...
504 error page containing HTML to the browser, which then gets parsed as if it's
XML.
...
[Prosody](http://prosody.im) (from version 0.10) and
ejabberd [(ejabberd_http_ws)](https://docs.ejabberd.im/admin/configuration/listen/#ejabberd_http_ws)
...
The `websocket_url` value you'll want to pass in to `converse.initialize` is `wss://mysite.com/xmpp-websocket`.
