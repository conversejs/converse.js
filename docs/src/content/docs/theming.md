...
the `classic` theme is used, as defined in [classic.scss](https://github.com/conversejs/converse.js/tree/master/src/shared/styles/themes/classic.scss).
...
The theme that Converse uses can be set via the [`theme`](/configuration/#theme) configuration
setting (and the [`dark_theme`](/configuration/#dark_theme) configuration setting for dark mode).

### How are themes applied?

When you set a value for the [`theme`](/configuration/#theme) configuration setting, Converse will add
...
To apply a theme, there then needs to be a CSS rule with a selector that matches the
`theme-dracula` class on the `converse-root` element.

If you take a look at the theme file [dracula.scss](https://github.com/conversejs/converse.js/tree/master/src/shared/styles/themes/dracula.scss)
...
CSS rule that matches the element you want to change.
...
The CSS files are generated from [Sass](http://sass-lang.com) files that end in `.scss` and
...
Converse uses [lit-html](https://lit.dev/docs/libraries/standalone-templates/) as HTML
...
Then I add a Webpack configuration and use [webpack aliases](https://webpack.js.org/configuration/resolve/#resolvealias)
