...
When you set a value for the [`theme`](/docs/configuration/#theme) configuration setting, Converse will add a class `theme-${api.settings.get('theme')}` on the `converse-root` DOM element.
