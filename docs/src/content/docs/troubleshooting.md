...
description: Tips for debugging Converse and solving common issues.
---
...
Converse has a [`loglevel`](/configuration/#loglevel) configuration setting which lets you turn on
...
[Strophe.js](http://strophe.im/), the underlying XMPP library which Converse
...
See for example the benchmarking tests in [spec/profiling.js](https://github.com/jcbrand/converse.js/blob/master/spec/profiling.js) which
can be used together with the [profiling features of Chrome](https://developer.chrome.com/devtools/docs/cpu-profiling) to find
...
roster, which eases this known performance bottleneck.
...
CORS stands for [Cross-Origin Resource Sharing (CORS)](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS)
...
but the domain of your HTTP file server (for [XEP-0363 HTTP File Upload](https://xmpp.org/extensions/xep-0363.html))
...
[CORS pre-flight request](https://developer.mozilla.org/en-US/docs/Web/HTTP/Methods/OPTIONS#Preflighted_requests_in_CORS)
...
A common complaint in the Converse chat room is that users are logged out when they reload the page.
...
Converse supports the [Web Authentication API](https://developer.mozilla.org/en-US/docs/Web/API/Web_Authentication_API)
...
credentials from the backend via the [`credentials_url`](/configuration/#credentials_url) setting.

#### Use BOSH instead of Websocket

[BOSH](https://xmpp.org/extensions/xep-0206.html) can be thought of
...
#### Use a browser with adequate support for the Web Auth API
...
The [desktop version of Converse](https://github.com/conversejs/converse-desktop)
...
This is a relatively generic [Skeletor](https://github.com/conversejs/skeletor) (or [Backbone](http://backbonejs.org/))
