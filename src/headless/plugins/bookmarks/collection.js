...
class Bookmarks extends Collection {
    ...
    async openBookmarkedRoom(bookmark) {
        if (api.settings.get('muc_respect_autojoin') && bookmark.get('autojoin')) {
            const groupchat = await api.rooms.create(bookmark.get('jid'), { 'nick': bookmark.get('nick') });
            groupchat.maybeShow();
        }
        return bookmark;
    }
        ...
        return api.sendIQ(stx`
            <iq type="set" from="${api.connection.get().jid}" xmlns="jabber:client">
                <pubsub xmlns="${Strophe.NS.PUBSUB}">
                    ${await this.getPubSubPublishNode()}
                    <publish-options>
    ...
    async createBookmarksFromStanza(stanza) {
        ...
        sizzle(sel, stanza).forEach(
            /** @param {Element} el */ (el) => {
                const jid = ns === Strophe.NS.BOOKMARKS2 ? el.parentElement.getAttribute('jid') : el.getAttribute('jid');
