        ...
        it(
            ...
            mock.initConverse(['discoInitialized'], {}, async function (_converse) {
                ...
                await u.waitUntil(() => {
                    ...
                    return (
                        IQ_stanzas.filter((iq) =>
                            sizzle('query[xmlns="http://jabber.org/protocol/disco#items"]', iq).pop(),
                        ).length > 0
                    );
                });
                ...
                stanza = IQ_stanzas.find(
                    (iq) =>
                        sizzle('iq[to="montague.lit"] query[xmlns="http://jabber.org/protocol/disco#items"]', iq)
                            .length,
                );
                ...
                expect(
                    entities.get('montague.lit').features.where({ 'var': 'http://jabber.org/protocol/disco#items' })
                        .length,
                ).toBe(1);
                expect(
                    entities.get('montague.lit').features.where({ 'var': 'http://jabber.org/protocol/disco#info' })
                        .length,
                ).toBe(1);
