...
describe('XEP-0202 Entity Time', function () {
    describe('Service discovery', function () {
        ...
        it(
            "doesn't advertise the urn:xmpp:time feature when send_entity_time is disabled",
    ...
    describe('Responding to time requests', function () {
        it(
            ...
            mock.initConverse(converse, ['statusInitialized'], {}, (_converse) => {
                ...
                _converse.api.connection.get()._dataRecv(mock.createRequest(_converse, time_iq));
        ...
        it(
            'returns service-unavailable when send_entity_time is disabled',
            mock.initConverse(converse, ['statusInitialized'], { send_entity_time: false }, (_converse) => {
                const time_iq = stx`
                    <iq from="romeo@montague.lit/orchard"
                        xmlns="jabber:client"
                        to="${_converse.jid}" id="time-1" type="get">
                ...
                expect(sent_stanza).toEqualStanza(stx`
                    <iq id="time-1" to="romeo@montague.lit/orchard" type="error" xmlns="jabber:client">
                        <error type="cancel">
    ...
    describe('Utility functions', function () {
        ...
        it(
            'formatRemoteTime formats time correctly',
            mock.initConverse(converse, ['statusInitialized'], {}, () => {
                const { formatRemoteTime } = converse.env.u.time;
                const utc = new Date('2026-03-16T12:00:00Z');

                expect(formatRemoteTime(utc, '+00:00')).toBe('12:00');
                expect(formatRemoteTime(utc, '+05:30')).toBe('17:30');
                expect(formatRemoteTime(utc, '-08:00')).toBe('04:00');
            }),
        );
    });
});
