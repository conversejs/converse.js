...
async function handleVCardUpdatePresence(pres) {
    await api.waitUntil('VCardsInitialized');
    const avatar_hash = sizzle(`x[xmlns="${Strophe.NS.VCARD_UPDATE}"] photo`, pres).pop()?.textContent;
    if (avatar_hash) {
...
export function registerPresenceHandler() {
    ...
    const connection = api.connection.get();
    presence_ref = connection.addHandler(handleVCardUpdatePresence, null, 'presence', null);
}
