...
import tplBookmarksPinList from './templates/pin-list.js';
import { RoomsList } from 'plugins/roomslist/view.js';
...
export class PinnedBookmarksView extends RoomsList {
    ...
    getRoomsToShow() {
        ...
        const rooms = chatboxes.filter(
            (m) => m.get('type') === constants.CHATROOMS_TYPE && !m.get('closed') && m.bookmark?.get('pinned')
        );
