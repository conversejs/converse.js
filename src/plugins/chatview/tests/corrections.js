    ...
    it(
        ...
        mock.initConverse(['chatBoxesFetched'], {}, async function (_converse) {
            ...
            await u.waitUntil(
                () =>
                    Array.from(view.querySelectorAll('.chat-msg__text')).filter(
                        (m) => m.textContent.replace(/<!-.*?->/g, '') === new_text,
                    ).length,
            );
            ...
            await u.waitUntil(
                () =>
                    Array.from(view.querySelectorAll('.chat-msg__text')).filter(
                        (m) => m.textContent === 'It is the east, and Juliet is the sun.',
                    ).length,
            );

            const messages = view.querySelectorAll('.chat-msg');
            expect(messages.length).toBe(3);
            expect(messages[0].querySelector('.chat-msg__text').textContent).toBe(
                'But soft, what light through yonder window breaks?',
            );
            expect(messages[1].querySelector('.chat-msg__text').textContent).toBe(
                'It is the east, and Juliet is the sun.',
            );
            expect(messages[2].querySelector('.chat-msg__text').textContent).toBe(
    ...
    describe('when received from someone else', function () {
        it(
            ...
            mock.initConverse(['chatBoxesFetched'], {}, async function (_converse) {
                ...
                expect(older_msgs[0].textContent.includes('But soft, what light through yonder airlock breaks?')).toBe(
                    true,
                );
                expect(view.model.messages.models.length).toBe(1);
            }),
