    ...
    describe('Discovering support', function () {
        it(
            ...
            mock.initConverse(['chatBoxesFetched'], {}, async function (_converse) {
                ...
                expect(
                    features[0].dataforms.where({ 'FORM_TYPE': { value: 'urn:xmpp:http:upload:0', type: 'hidden' } })
                        .length,
                ).toBe(1);
            }),
        );
            ...
            describe('when clicked and a file chosen', function () {
                it(
                    ...
                    mock.initConverse(['chatBoxesFetched'], {}, async (_converse) => {
                        ...
                        await u.waitUntil(
                            () =>
                                IQ_stanzas.filter((iq) => iq.querySelector('iq[to="upload.montague.tld"] request'))
                                    .length,
                        );
                        ...
                        spyOn(XMLHttpRequest.prototype, 'send').and.callFake(async function () {
                            ...
                            await u.waitUntil(
                                () => view.querySelector('.chat-content progress').getAttribute('value') === '0.5',
                            );
                            message.set('progress', 1);
                            await u.waitUntil(
                                () => view.querySelector('.chat-content progress').getAttribute('value') === '1',
                            );
                            message.save({
                ...
                it(
                    ...
                    mock.initConverse(['chatBoxesFetched'], {}, async function (_converse) {
                        ...
                        expect(entities.get(_converse.domain).features.length).toBe(2);
                        expect(entities.get(_converse.domain).identities.length).toBe(1);
                        expect(entities.pluck('jid')).toEqual([
                            'montague.lit',
                            'romeo@montague.lit',
                            'upload.montague.lit',
                        ]);
            ...
            it(
                ...
                mock.initConverse(['chatBoxesFetched'], {}, async function (_converse) {
                    ...
                    spyOn(XMLHttpRequest.prototype, 'send').and.callFake(async () => {
                        ...
                        expect(view.querySelector('.chat-content .chat-msg__text').textContent).toBe(
                            'Uploading file: my-juliet.jpg, 23.46 kB',
                        );
                        promise.resolve();
                    });
                    api.connection.get()._dataRecv(mock.createRequest(stanza));
                    ...
                    expect(XMLHttpRequest.prototype.setRequestHeader.calls.all()[1].args[1]).toBe(
                        'Basic Base64String==',
                    );
                }),
            );
