...
const { $msg, Strophe, u, stx } = converse.env;
        ...
        it("parses incoming reply messages correctly",
            ...
            _converse.handleMessageStanza(
                $msg({
                    ...
                    'type': 'chat',
                    'id': original_id
                }).c('body').t('Original message').up()
                .c('active', {'xmlns': 'http://jabber.org/protocol/chatstates'}).tree()
            );
            await u.waitUntil(() => view.querySelectorAll('.chat-msg__text').length === 1);

            // Now send a reply message
            const reply_stanza = $msg({
                'from': contact_jid,
                'to': api.connection.get().jid,
                'type': 'chat',
                'id': u.getUniqueId()
            }).c('body').t('This is a reply').up()
            .c('reply', {'xmlns': 'urn:xmpp:reply:0', 'id': original_id, 'to': api.connection.get().jid}).up()
            .c('active', {'xmlns': 'http://jabber.org/protocol/chatstates'}).tree();
            ...
            await u.waitUntil(() => view.querySelector('.chat-msg__reply-context'));
            const replyContext = view.querySelector('.chat-msg__reply-context');
