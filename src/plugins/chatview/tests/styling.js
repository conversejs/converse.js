...
describe('An incoming chat Message', function () {
    it(
        ...
        mock.initConverse(['chatBoxesFetched'], {}, async function (_converse) {
            ...
            const msg = $msg({
                ...
                .t(msg_text)
                .up()
                .c('unstyled', { 'xmlns': 'urn:xmpp:styling:0' })
                .tree();
    ...
    it(
        'can have styling disabled via the allow_message_styling setting',
        mock.initConverse(['chatBoxesFetched'], { 'allow_message_styling': false }, async function (_converse) {
            const { api } = _converse;
            const include_nick = false;
            await mock.waitForRoster(_converse, 'current', 2, include_nick);
            await mock.openControlBox(_converse);
    ...
    it(
        ...
        mock.initConverse(['chatBoxesFetched'], {}, async function (_converse) {
            ...
            msg_el = Array.from(view.querySelectorAll('converse-chat-message-body')).pop();
            expect(msg_el.innerHTML.replace(/<!-.*?->/g, '')).toBe(
                '<blockquote><blockquote>This is doubly quoted text</blockquote></blockquote>',
            );
            ...
            msg_el = Array.from(view.querySelectorAll('converse-chat-message-body')).pop();
            await u.waitUntil(
                () => msg_el.innerHTML.replace(/<!-.*?->/g, '') === 'hello world &gt; this is not a quote',
            );
            ...
            msg_text =
                '> This next line will be purposefully empty, and include no trailing space.\n>\n> This is the end of this quote.';
            msg = mock.createChatMessage(_converse, contact_jid, msg_text);
            await _converse.handleMessageStanza(msg);
            ...
            msg_text =
                '> Quotes shouldn’t prevent multiple lines from being collapsed together.\n> \n> \n> \n> Like so.';
            msg = mock.createChatMessage(_converse, contact_jid, msg_text);
            await _converse.handleMessageStanza(msg);
        ...
        mock.initConverse(['chatBoxesFetched'], {}, async function (_converse) {
            ...
            _converse.handleMessageStanza(
                $msg({
                    ...
                    .t(`A\nhttps://conversejs.org\n\nhttps://opkode.com`)
                    .up()
                    .c('replace', { 'id': msg_id, 'xmlns': 'urn:xmpp:message-correct:0' })
                    .tree(),
            );
            await new Promise((resolve) => view.model.messages.once('rendered', resolve));
