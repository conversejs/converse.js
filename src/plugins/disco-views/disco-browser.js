...
export class DiscoBrowser extends CustomElement {
    ...
    render() {
        ...
        return html`<nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ${this._entity_jids.map(
                        (jid, index) => html`
                            <li class="breadcrumb-item">
                                <a href="#" @click="${(ev) => this.handleBreadcrumbClick(ev, index)}">${jid}</a>
                            </li>
                        `
                    )}
                </ol>
                            ...
                            ? html`<h5 class="mt-3">${__('Items')}</h5>
                                  <ul>
                                      ${items.map(
                                          (i) =>
                                              html`<li>
                                                  <a @click="${(ev) => this.addEntityJID(ev, i)}"
                                                      >${this.renderItem(i)}</a
                                                  >
                                              </li>`
                                      )}
    ...
    async getDiscoInfo() {
        const entity = await api.disco.entities.get(this._entity_jids[this._entity_jids.length - 1]);
