...
export default (el) => {
    ...
    const num_unread = chats.reduce((/** @type {number} */ acc, chat) => acc + chat.get('num_unread'), 0);
    const num_minimized = chats.reduce((/** @type {number} */ acc, chat) => acc + (chat.get('hidden') ? 1 : 0), 0);
    const collapsed = el.minchats.get('collapsed');

    return html`<div id="minimized-chats" class="${chats.length ? '' : 'hidden'}">
        <button type="button" class="btn btn-primary" @click=${(/** @type {Event} */ ev) => el.toggle(ev)}>
            ...
            ${chats.map(
                (chat) =>
                    html`<converse-minimized-chat
                        ...
                        type=${chat.get('type')}
                        num_unread=${chat.get('num_unread')}
