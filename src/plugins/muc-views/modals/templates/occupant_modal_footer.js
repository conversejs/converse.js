...
export default (el) => {
    ...
    const handleOwner = async (ev) => {
        const confirmed = await _converse.api
            .confirm(
    ...
    const memberButton = (memberText) =>
        html`<button class="btn btn-primary" @click=${handleMember}>${memberText}</button>`;
    ...
    const canBan = {
        ...
        'member': ['none', 'member'].includes(affiliation)
            ? banButton
            : ['visitor', 'none', 'participant'].includes(role)
            ? banButton
            : null,
    }[ownAffiliation];
    ...
    const canKick = {
        ...
        'member':
            ['none', 'member'].includes(affiliation) && ![' none', 'moderator'].includes(role) ? kickButton : null,
    }[ownAffiliation];
    ...
    const canMute = {
        ...
        'member':
            ['none', 'member'].includes(affiliation) && !['visitor', 'moderator'].includes(role) ? muteButton : null,
    }[ownAffiliation];
    ...
    const canOwner = { 'owner': ['none', 'member', 'admin'].includes(affiliation) ? ownerButton : null }[
        ownAffiliation
    ];
    ...
    const determineApplicable = function (command) {
        switch (command) {
            ...
            case 'block': {
                return blocking_plug && jid && !api.blockedUsers().has(jid) ? blockButton : null;
            }
            case 'unblock': {
                return blocking_plug && jid && api.blockedUsers().has(jid) ? unblockButton : null;
            }
            default: {
                return null;
            }
    ...
    return html`${until(
        buttons?.then((buttons) => (buttons ? html`<div class="modal-footer">${buttons}</div>` : '')),
        ''
    )}`;
};
