...
function getRoleHelpText(role) {
    if (role === 'moderator') {
        return __(
            'Moderators are privileged users who can change the roles of other users (except those with admin or owner affiliations.',
        );
...
/**
 * @param {import('../types').Affiliation} aff
 */
function getAffiliationHelpText(aff) {
    if (aff === 'owner') {
        ...
        return __(
            'Admin is the 2nd highest affiliation. Admins can modify roles and affiliations of all other users except owners.',
        );
    } else if (aff === 'outcast') {
...
const role_option = (o) => html`
    <option value="${o.item || ''}" ?selected=${o.item === o.role} title="${getRoleHelpText(o.item)}">${o.item}</option>
`;

const affiliation_option = (o) => html`
    <option value="${o.item || ''}" ?selected=${o.item === o.affiliation} title="${getAffiliationHelpText(o.item)}">
        ${o.item}
    </option>
`;

const tplRoleFormToggle = (o) =>
    html` <a
        href="#"
        data-form="converse-muc-role-form"
...
/**
 * @param {import('../modtools').default} el
 */
function tplRoleListItem(el, o) {
    return html` <li class="list-group-item" data-nick="${o.item.nick}">
        <ul class="list-group">
            ...
            <li class="list-group-item">
                <div>
                    <strong>Role:</strong> ${o.item.role} ${o.assignable_roles.length ? tplRoleFormToggle(o) : ''}
                </div>
                ${o.assignable_roles.length
                    ? html`<converse-muc-role-form
                          class="hidden"
                          .muc=${el.muc}
                          jid=${o.item.jid}
                          role=${o.item.role}
...
const affiliation_form_toggle = (o) =>
    html` <a
        href="#"
        data-form="converse-muc-affiliation-form"
        class="toggle-form right"
        color="var(--secondary-color)"
        @click=${o.toggleForm}
    >
...
const affiliation_list_item = (el, o) => html`
    <li class="list-group-item" data-nick="${o.item.nick}">
        <ul class="list-group">
            ...
            <li class="list-group-item">
                <div>
                    <strong>Affiliation:</strong> ${o.item.affiliation}
                    ${o.assignable_affiliations.length ? affiliation_form_toggle(o) : ''}
                </div>
                ${o.assignable_affiliations.length
                    ? html`<converse-muc-affiliation-form
                          class="hidden"
                          .muc=${el.muc}
                          jid=${o.item.jid}
                          affiliation=${o.item.affiliation}
...
const tplNavigation = (o) => html`
    <ul class="nav nav-pills justify-content-center">
        <li role="presentation" class="nav-item">
            <a
                class="nav-link ${o.tab === 'affiliations' ? 'active' : ''}"
                id="affiliations-tab"
        ...
        <li role="presentation" class="nav-item">
            <a
                class="nav-link ${o.tab === 'roles' ? 'active' : ''}"
                id="roles-tab"
                href="#roles-tabpanel"
...
export default (el, o) => {
    ...
    return html` ${o.alert_message
            ? html`<div class="alert alert-${o.alert_type}" role="alert">${o.alert_message}</div>`
            : ''}
        ...
        <div class="tab-content">
            ${o.queryable_affiliations.length
                ? html` <div
                      class="tab-pane tab-pane--columns ${o.tab === 'affiliations' ? 'active' : ''}"
                      id="affiliations-tabpanel"
                      role="tabpanel"
                      ...
                      <form class="converse-form query-affiliation" @submit=${o.queryAffiliation}>
                          ...
                          <div>
                              ...
                              <div class="row">
                                  <div class="col">
                                      <select class="form-select select-affiliation" name="affiliation">
                                          ${o.queryable_affiliations.map((item) =>
                                              affiliation_option(Object.assign({ item }, o)),
                                          )}
                                      </select>
                                  </div>
                                  <div class="col">
                                      <input
                                          ...
                                          name="users_with_affiliation"
                                          value="${i18n_show_users}"
                                      />
                                  </div>
                              </div>
                              <div class="row">
                                  <div class="col mt-3">
                                      ${Array.isArray(o.users_with_affiliation) && o.users_with_affiliation.length > 5
                                          ? html`<input
                                                class="form-control"
                                                .value="${o.affiliations_filter}"
                                                @keyup=${o.filterAffiliationResults}
                                                type="text"
                                                name="filter"
                                                placeholder="${i18n_filter}"
                                            />`
                                          : ''}
                                  </div>
                              </div>
                              ...
                              ${getAffiliationHelpText(o.affiliation)
                                  ? html`<div class="row">
                                        <div class="col pt-2">
                                            <p class="helptext pb-3">${getAffiliationHelpText(o.affiliation)}</p>
                                        </div>
                                    </div>`
                                  : ''}
                          </div>
                      ...
                      <div class="scrollable-container">
                          <ul class="list-group list-group--users">
                              ...
                              ${o.users_with_affiliation instanceof Error
                                  ...
                                  : (o.users_with_affiliation || []).map((item) =>
                                        (item.nick || item.jid).match(new RegExp(o.affiliations_filter, 'i'))
                                            ? affiliation_list_item(el, Object.assign({ item }, o))
                                            : '',
                                    )}
                          </ul>
                      </div>
            ...
            ${o.queryable_roles.length
                ? html` <div
                      class="tab-pane tab-pane--columns ${o.tab === 'roles' ? 'active' : ''}"
                      id="roles-tabpanel"
                      role="tabpanel"
                      ...
                      <form class="converse-form query-role" @submit=${o.queryRole}>
                          ...
                          <div>
                              ...
                              <div class="row">
                                  <div class="col mt-3">
                                      ${Array.isArray(o.users_with_role) && o.users_with_role.length > 5
                                          ? html`<input
                                                class="form-control"
                                                .value="${o.roles_filter}"
                                                @keyup=${o.filterRoleResults}
                                                type="text"
                                                name="filter"
                                                placeholder="${i18n_filter}"
                                            />`
                              ...
                              ${getRoleHelpText(o.role)
                                  ? html`<div class="row">
                                        <div class="col pt-2">
                                            <p class="helptext pb-3">${getRoleHelpText(o.role)}</p>
                                        </div>
                                    </div>`
                                  : ''}
                          </div>
                      ...
                      <div class="scrollable-container">
                          <ul class="list-group list-group--users">
                              ...
                              ${o.users_with_role && o.users_with_role.length === 0
                                  ? html`<li class="list-group-item">${i18n_no_users_with_role}</li>`
                                  : ''}
                              ${(o.users_with_role || []).map((item) =>
                                  item.nick.match(o.roles_filter)
                                      ? tplRoleListItem(el, Object.assign({ item }, o))
                                      : '',
                              )}
                          </ul>
                      </div>
