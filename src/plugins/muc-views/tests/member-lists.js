    ...
    describe('upon being entered', function () {
        ...
        it(
            ...
            mock.initConverse([], { 'muc_fetch_members': true }, async function (_converse) {
                ...
                const model = await mock.openAndEnterMUC(
                    _converse,
                    muc_jid,
                    'romeo',
                    [],
        ...
        describe('when fetching the member lists', function () {
            it(
                ...
                mock.initConverse([], { 'muc_fetch_members': true }, async function (_converse) {
                    ...
                    expect(view.model.occupants.findWhere({ 'jid': _converse.bare_jid }).get('affiliation')).toBe(
                        'owner',
                    );
                    expect(view.model.occupants.findWhere({ 'jid': 'hag66@shakespeare.lit' }).get('affiliation')).toBe(
                        'member',
                    );
                }),
            );
