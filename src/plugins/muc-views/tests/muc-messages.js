...
describe('A Groupchat Message', function () {
    ...
    it(
        'can contain a chat state notification and will still be shown',
    ...
    it(
        'can not be expected to have a unique id attribute',
        mock.initConverse(['chatBoxesFetched'], {}, async function (_converse) {
            const muc_jid = 'lounge@montague.lit';
            await mock.openAndEnterMUC(_converse, muc_jid, 'romeo');
            const view = _converse.chatboxviews.get(muc_jid);
    ...
    it(
        'is ignored if it has the same stanza-id of an already received one',
        mock.initConverse([], {}, async function (_converse) {
    ...
    it(
        "keeps track of the sender's role and affiliation",
        mock.initConverse([], {}, async function (_converse) {
            const muc_jid = 'lounge@montague.lit';
            ...
            expect(sizzle('.chat-msg', view).pop().classList.value.trim()).toBe(
                'message chat-msg groupchat chat-msg--with-avatar moderator owner',
            );
            let presence = stx`
            ...
            expect(sizzle('.chat-msg', view).pop().classList.value.trim()).toBe(
                'message chat-msg groupchat chat-msg--with-avatar participant member',
            );
            ...
            const occupant = await u.waitUntil(
                () => view.model.messages.filter((m) => m.get('type') === 'groupchat')[2].occupant,
            );
            ...
            await u.waitUntil(
                () =>
                    sizzle('.chat-msg', view).pop().classList.value.trim() ===
                    'message chat-msg groupchat chat-msg--with-avatar moderator owner',
            );
    ...
    it(
        'will be shown as received and have its body updated upon MUC reflection',
        mock.initConverse([], {}, async function (_converse) {
