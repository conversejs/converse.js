            ...
            mock.initConverse(['chatBoxesFetched'], {}, async function (_converse) {
                ...
                await u.waitUntil(() =>
                    view
                        ...
                        .textContent.includes('romeo is no longer a moderator'),
                );
                const retraction_stanza = await sendAndThenRetractMessage(_converse, view);
                await u.waitUntil(() => view.querySelectorAll('.chat-msg--retracted').length === 1, 1000);
                ...
                expect(errmsg.textContent.trim()).toBe(
        ...
        it(
            ...
            mock.initConverse(
                ...
                async function (_converse) {
                    ...
                    await view.model.handleMessageStanza(reflection_stanza);
                    await u.waitUntil(
                        () => view.querySelectorAll('.chat-msg__body.chat-msg__body--received').length,
                        500,
                    );
                    expect(view.model.messages.length).toBe(1);
                    expect(view.model.messages.at(0).get('editable')).toBe(true);
                    ...
                    const stanza = await u.waitUntil(() =>
                        sent_IQs.filter((iq) => iq.querySelector('iq moderate')).pop(),
                    );
