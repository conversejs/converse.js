...
describe('An OMEMO encrypted message', function () {
    ...
    it(
        ...
        mock.initConverse(['chatBoxesFetched'], {}, async function (_converse) {
            ...
            await u.waitUntil(
                () => _converse.api.connection.get().sent_stanzas.filter((s) => s.nodeName === 'message').length === 3,
            );
            ...
            expect(view.model.messages.length).toBe(2);
            expect(view.querySelectorAll('.chat-msg__body')[1].textContent.trim()).toBe(
                'This is an edited encrypted message from the contact',
            );
...
describe('An OMEMO encrypted MUC message', function () {
    ...
    it(
        ...
        mock.initConverse(['chatBoxesFetched'], {}, async function (_converse) {
            ...
            await u.waitUntil(
                () => _converse.api.connection.get().sent_stanzas.filter((s) => s.nodeName === 'message').length === 2,
            );
            const msg = _converse.api.connection.get().sent_stanzas.pop();
