 ...
 * - Users can have multiple reactions per message
 */
...
converse.plugins.add('converse-reactions', {
    ...
    async onReactionReceived (stanza, reactions_element) {
        ...
        const all_chatboxes = await api.chatboxes.get();
        for (const cb of all_chatboxes) {
    ...
    onReactionSelected (ev) {
        ...
        const message = target.closest('converse-chat-message') || target.getRootNode()?.host;
        if (message && detail?.emoji) {
            this.sendReaction(message.model, detail.emoji);
        }
    },
