...
const chooseReactionViaUI = async (view, msgid, emoji) => {
    ...
    const emoji_btn = Array.from(picker_el.querySelectorAll('.reaction-item'))
        .find(el => el.textContent.trim() === emoji);
    if (emoji_btn) {
        emoji_btn.click();
        return;
    }
    ...
    const emoji_link = await u.waitUntil(() => Array.from(msg_el.querySelectorAll('.emoji-picker li.insert-emoji a'))
        .find(el => el.textContent.trim() === emoji));
...
describe("Message Reactions (XEP-0444)", function () {
    ...
    describe("sending reactions in a 1:1 chat", function () {
        ...
        it("sends a correct XEP-0444 stanza when a reaction is added",
            mock.initConverse(['chatBoxesFetched'], {}, async function (_converse) {
                ...
                const sent_stanza = api.connection.get().send.calls.all().map(
                    c => c.args[0] instanceof Element ? c.args[0] : c.args[0].tree()
        ...
        it("toggles off a reaction when the same emoji is sent again",
            mock.initConverse(['chatBoxesFetched'], { popular_reactions }, async function (_converse) {
                ...
                const all_stanzas = api.connection.get().send.calls.all().map(
                    c => c.args[0] instanceof Element ? c.args[0] : c.args[0].tree()
    ...
    describe("receiving reactions in a 1:1 chat", function () {
        ...
        it("applies an incoming reaction stanza to the correct message",
            mock.initConverse(['chatBoxesFetched'], {}, async function (_converse) {
                await mock.waitForRoster(_converse, 'current', 1);
                const contact_jid = mock.cur_names[0].replace(/ /g, '.').toLowerCase() + '@montague.lit';
                await mock.openChatBoxFor(_converse, contact_jid);
        ...
        it("replaces a user's previous reactions when receiving a new reaction stanza",
            mock.initConverse(['chatBoxesFetched'], {}, async function (_converse) {
                await mock.waitForRoster(_converse, 'current', 1);
                const contact_jid = mock.cur_names[0].replace(/ /g, '.').toLowerCase() + '@montague.lit';
                await mock.openChatBoxFor(_converse, contact_jid);
        ...
        it("removes all reactions when receiving an empty reaction set",
            mock.initConverse(['chatBoxesFetched'], {}, async function (_converse) {
                await mock.waitForRoster(_converse, 'current', 1);
                const contact_jid = mock.cur_names[0].replace(/ /g, '.').toLowerCase() + '@montague.lit';
                await mock.openChatBoxFor(_converse, contact_jid);
    ...
    describe("in a MUC", function () {
        ...
        it("sends a groupchat reaction stanza targeting the MUC JID",
            mock.initConverse(['chatBoxesFetched'], { popular_reactions }, async function (_converse) {
                const { api } = _converse;
                await mock.waitForRoster(_converse, 'current', 1);
        ...
        it("accumulates reactions from multiple participants via incoming stanzas",
            mock.initConverse(['chatBoxesFetched'], {}, async function (_converse) {
        ...
        it("preserves other users' reactions when one user updates theirs",
            mock.initConverse(['chatBoxesFetched'], {}, async function (_converse) {
                await mock.waitForRoster(_converse, 'current', 0);
