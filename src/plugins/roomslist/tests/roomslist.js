    ...
    it(
        ...
        mock.initConverse(
            ...
            async function (_converse) {
                ...
                expect(getComputedStyle(muc_initials_el).backgroundColor).toBe(
                    getComputedStyle(initials_el).backgroundColor,
                );
                ...
                const muc_heading_avatar = await u.waitUntil(() =>
                    muc_el.querySelector(`converse-muc-heading converse-avatar svg image`),
                );

                const { default_avatar_image, default_avatar_image_type } = _converse;
                expect(muc_heading_avatar.getAttribute('href')).toBe(
                    `data:${default_avatar_image_type};base64,${default_avatar_image}`,
                );

                const list_el_image = rooms_list.querySelector('converse-avatar svg image');
                expect(list_el_image.getAttribute('href')).toBe(
                    `data:${default_avatar_image_type};base64,${default_avatar_image}`,
                );
            },
        ),
