...
import { parseXMPPURI } from 'shared/xmpp-uri.js';
...
/**
 * Vet an incoming URI and give a query-less one the action it implies.
 *
 * A bare `xmpp:<jid>` names no query action. Rendered in a message such a link
 * is deliberately left to the browser's own handler, but here we *are* that
 * handler, so there is nothing to fall through to; a bare JID means "talk to
 * this entity", so it becomes the `message` action.
 *
 * @param {string} uri
 * @returns {string|null} The URI to dispatch, or null if it names no valid JID.
 */
function normalizeURI(uri) {
    const { jid, action } = parseXMPPURI(uri);
    if (!u.isValidJID(jid)) return null;

    return action ? uri : `${uri}?message`;
}
...
export async function routeXMPPURI() {
    const raw = getURIFromHash();
    if (!raw || dispatching || !api.connection.connected()) return false;

    dispatching = true;
    try {
        const uri = normalizeURI(raw);
        return uri ? await dispatchXMPPURI(uri) : false;
    } finally {
        dispatching = false;
        clearActionHash();
    }
