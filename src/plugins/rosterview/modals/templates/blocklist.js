...
export default (el) => {
    if (el.blocklist.length) {
        ...
        return html`
            ${el.blocklist.length > 3 ? html`<p>${__('Number of blocked users:')} ${el.blocklist.length}</p>` : ''}
            ...
            <form @submit="${(ev) => el.unblockUsers(ev)}">
                ...
                <button type="submit" class="btn btn-danger mt-3">${__('Unblock selected users')}</button>
            </form>
        `;
    } else {
