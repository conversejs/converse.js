    ...
    it(
        ...
        mock.initConverse([], {}, async function (_converse) {
            ...
            const sent_stanza = await u.waitUntil(() =>
                sent_IQs.filter((iq) => sizzle(`iq[type="set"] query[xmlns="${Strophe.NS.ROSTER}"]`, iq).length).pop(),
            );
    ...
    it(
        ...
        mock.initConverse(
            ...
            async function (_converse) {
                ...
                const suggestions = Array.from(modal.querySelectorAll('.suggestion-box li')).map((li) =>
                    li.textContent.trim(),
                );
    ...
    it(
        ...
        mock.initConverse(
            ...
            async function (_converse) {
                ...
                const providerFetchCalls = fetchSpy.calls.all().filter((call) => call.args[0]?.includes?.('providers'));
